...
2. **壳保持薄**：前端框架已裁定向 React 生态收敛（ADR-0008：Vite + React + TS + Tailwind + shadcn/ui），约束更新为——**仍禁数据库 / IPC 总线 / 领域服务**；前端框架仅限白名单（AGENTS §4.4），不引入 React 生态外框架
...
| 引入数据库 / IPC 总线 / 领域服务 | 壳保持薄；前端框架已由 ADR-0008 裁定向 React 生态收敛，不在此列 |
