...
>
> **工程前置（4.3 开工前）**：IPC 三处同步一致性自检自动化 spike——当前三处同步（build.rs + capabilities + lib.rs，AGENTS §7）漏一处即静默失败，靠登记提醒防；4.3/4.4 将新增大量 IPC 命令，开工前把「人肉纪律」升级为机器闸门（如 build.rs 断言 capabilities ⊆ 已实现命令，或启动诊断自检）。
