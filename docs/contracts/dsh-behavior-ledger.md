...
| 22 | **dsh 安装包自带的官方实验层**（optional bundle：谁自带、怎么开关、能不能卸） | `official_catalog.rs::installation_provided`（**安装清单声明 ∨ 运行时在册**双信号探测 `<engines>/dsh-runtime/node_modules/`；原 `installation_shipped`，2026-09-22 加固改名）＋ `CapabilityView.{shippedByDsh, legacyCopy}`；前端经 `lib/pluginCatalog.ts::dockCuratedCaps` 过滤后**整体不呈现**（2026-09-20 起壳不再消费 `legacyCopy`，ShippedPane 已随之退役） | dsh **0.1.6-alpha.2** @ 上游 `ddfc45fb`（2026-09-17 源锚 + 本机正式档引擎实测）：① **自带清单 = 上游 `OPTIONAL_BUNDLES`**（`packages/boot/app-boot/src/profile.ts`）：`@deepseek-ai/dsh-experimental-agent-team-profile`、`…-web-profile`；判据是"**是 `apps/cli` 的运行时依赖 + 声明 `dsh.bundle.patch` + 没有任何出厂模板选中它**"，由 `scripts/` 的静态门与打包验收门共同保证（设计笔记 `.agents/notes/implemented/process/2026-09-15-shipped-optional-bundles.md`）；② **语义**：随安装包下载、**默认关**、在 dsh 自己的插件页开关、`optional: true` 且 **`removable: false`**（`packages/boot/plugin-manager/src/index.ts:203` 的 `OPTIONAL_BUNDLES.includes(name)`；`manager-store.ts:73` 注释原文 "official, off until selected, never removable"）；③ **该笔记明确否决**"由某个面板按名字从 registry 安装官方 bundle"这条替代路径（理由：开关那一刻要联网 + 每次发布要钉一个版本）——**本模块原来的做法正是它**；④ 真机实测：正式档引擎 `dsh 0.1.6-alpha.2` 的 `node_modules/@deepseek-ai/` 下确有这两个包（`0.1.6-alpha.2`），且用户 `~/.dsh/profiles/web` 的 `dsh.profile.bundles` 含它们、而 **profile 依赖里没有**（＝ dsh 提供的层）；dev 档引擎 `0.1.6-alpha.1` 两者皆无；⑤ `auto-review` **不在**自带清单（同笔记原文："a published experimental package the page's install guide names as its example, not an optional bundle"）；`browser-use` / `computer-use` 亦未自带（仍是"要装才有"的 provider 包）。**2026-09-22 复核（dsh 0.1.7-rc.1 本机正式档引擎实测）**：⑥ `OPTIONAL_BUNDLES` = `@deepseek-ai/dsh-experimental-agent-team-profile` ＋ **新增** `@deepseek-ai/dsh-experimental-voice-input-bundle`（`dsh-app-boot/lib/index.js:542` 硬编码数组；与 `@deepseek-ai/dsh` package.json 的 `dependencies` 精确一致——上游 `listBundles` 同样以 installAnchor 的 `dependencies` 判 `removable`，`dsh-plugin-manager/lib/index.js:1393-1403`）；⑦ **`agent-team-web-profile` 退役**（npm 终版 `0.1.6-alpha.2`，0.1.7 起无此包；`agent-team-profile` 自述 "Agent Teams collaboration, tools, and Web UI in one experimental bundle"——上游把两包收拢成一个 bundle）；⑧ `auto-review` / `browser-use` / `computer-use` 仍**不在**安装 `dependencies` 里（仍是"要装才有"，继续由 dock 策展）；⑨ `not-removable` 拒绝码与 `optional`/`removable` 语义仍在（`dsh-plugin-manager/lib/index.js:1780`）。**教训（已修，ADR-0020 §2.8 2026-09-22 修订）**：目录引用退役包 ⇒ "该能力每个包都官方提供"的全量判据恒不成立 ⇒ 面板为官方已接管的能力重新摆出操作入口（默认首档还会去装一个不存在的包版本）。**升级复核项**：`@deepseek-ai/dsh` 的 `dependencies` 里是否出现新的 `@deepseek-ai/dsh-experimental-*`（等价于 `OPTIONAL_BUNDLES` 增项）→ 出现即进入"官方提供"集合（判据是安装侧事实，enumerate 自动跟上），但要复核本模块的文案与边界仍成立；`optional`/`removable` 语义与 `not-removable` 拒绝码是否仍在；自带层是否开始支持"随包一起装 companion 层"（笔记明写当前**不**自动选 companion）；**WSL 客体世界的探测缺口（另案）**：客体 dsh 是 `pnpm add --global` 装在客体 `~/.dsh-dock/engines`（ADR-0004 §8），而当前探测只读宿主 `<engines>/dsh-runtime`——宿主机引擎比客体新时会"谎称自带"把唯一入口关掉（违背后的保守方向），客体世界对齐待另一次增量 | 2026-09-17（2026-09-22 随 dsh 0.1.7-rc.1 复核更新） |
| 23 | **用户 patch 的层结构（两个用户层 = 两种生效范围）与 MCP 条目的加载契约** | `mcp.rs::{McpScope, list_mcp_servers, save_mcp_server, delete_mcp_server, parse_mcp_servers_in_layer}`；`mcp_probe.rs::probe_stdio`（PATH / `cwd` 复现）；前端 `McpManager.tsx` 的范围徽标与跨层冲突告警 | dsh **0.1.6-alpha.2** @ 引擎 `engines/dsh-runtime/node_modules/@deepseek-ai/`（2026-09-18 源锚 + `--dump-config` 实机实测）：① **层顺序** = `dsh-app-boot/lib/index.js:1005 readProfilePatches`：各 bundle 层 → profile 层（`profiles/<名>/cordis.patch.yml`，由 `loadProfileDirectory` 的 `patches` 返回，`:929-930`）→ **home 级全局层**（`join(context.home, "cordis.patch.yml")`，`:1010`）→ `context.overlays`；② **实测坐实"全局层对所有 profile 生效"**：`dsh --profile web --dump-config` 的输出在 `# == /Users/guan/.dsh/profiles/web/cordis.patch.yml` 段之后，紧跟 `# == /Users/guan/.dsh/cordis.patch.yml` 段的 `mcp-tempad-dev`；③ **`desktop` profile 被 CLI 拒**（`lib/bin.js:38 rejectElectronProfile`：`profile "desktop" is managed exclusively by the Electron application`）⇒ 该 profile 的行表/组合树**无法**用 dump-config 复核，只能读文件；④ **空层会砖**：`parsePatchList`（`:2219-2230`）对"解析结果不是顶层数组"**直接 throw**，纯注释文件经 js-yaml 得 `null` ⇒ 该层所在 profile 起不来（本机 js-yaml 实测 `Array.isArray(null) === false`）——空层必须写 `[]`；⑤ **`mcp-client` 一个实例只连一台服务器**（`src/index.ts:3-5` 注释 + `Config` union `:119-142`），字段扁平：`serverName` / `transport` / `command`+`args`+`env`+`cwd` 或 `url`+`headers`，**没有 `mcpServers` 这个键**；`serverName` 受 `SERVER_NAME_PATTERN = /^[A-Za-z0-9_-]{1,32}$/`（`:40`）约束，不合规**行能写进去但插件加载被 schema 拒**（＝界面"已启用"而模型侧无工具）；`cwd` 缺省 `''`（`:786 z.string().default("")`）；⑥ **同名 `serverName` 是加载期预留**（`:160-176` `activeServerNames` + `ctx.effect`，在 `startConnection` **之前**）⇒ 两层同名时两条都会插入、**先加载的占住名字、后加载的那条抛 `serverName "x" is already in use`**（不是覆盖）；⑦ **宿主 PATH 极窄**（环境观测，非源码）：Harness 主进程 `PATH=/usr/bin:/bin:/usr/sbin:/sbin`（`ps eww`），壳 `engines/bin` 只有 `dsh`/`node`/`pnpm`（**无 npx**）⇒ `command: npx` 在两个宿主下都 ENOENT（Node 24 `spawnSync` 实测，窄 PATH 下 `npx` 恒 ENOENT），可行写法 = 绝对路径 node + 绝对路径入口脚本 + 显式 `cwd`（本机 tempad-dev 即此形态，定义在全局层）；⑧ **用户 patch 里的 `!!js` 标签值**：cordis loader 在加载期**求值**（与复现点 13 的 `disabledOf` 支持 `!!js` @ 377-379 同源），上游用它传"从环境变量取的 secret 引用"；而壳侧 `serde_yaml 0.9.34` 解析到 `Value` 时**静默丢掉标签**（本机实测：`!!js x` 读回就是字符串）⇒ 壳的任何"解析→改→重序列化"都会把表达式写成字面量，**保真只能走文本**（`expr` 由原文判定 + 启停走 `toggle_disabled_in_text`；口径见 ADR-0027 §7.2）。**升级复核项**：`readProfilePatches` 的层集合与顺序；`parsePatchList` 是否改为容忍 `null`；`mcp-client` 的 `Config` 形状（是否改为单实例多服务器 / 放宽 `serverName`）；`StdioConfig.cwd` 缺省值；`rejectElectronProfile` 的 profile 名单；**cordis 是否仍支持 `!!js` 求值**（若移除，`expr` 一整套保真口径可回收） | 2026-09-18（⑧ 补记于 2026-09-19） |
...
- 2026-09-24 新增复现点 24（插件侧问题不砖启动；ADR-0031 删除安全模式的依据）：维护者
  ...
  前提随之消失，ADR-0031 删按钮/横幅/记账、保留「备份并放空」兜底（`quarantine_patch`）。
  注意：2026-09-16 事故的 exit 1 记录未删（历史事实），以本条复核结论为准。
