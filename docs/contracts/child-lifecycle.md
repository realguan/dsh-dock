...
| 9 | **真 dsh** 被硬杀后收口（走生产 `spawn_dsh`） | 真机锚（`#[ignore]`） | ✅ 新增；已用**变异测试**证明会红 |
| 10 | 锁判定三态：只有 `WouldBlock` 授权收口 | 单测 | ✅ 新增 |
| 11 | **引导真的调用了清扫**（且早于派发执行器） | 源码文本闸门 | ✅ 新增；删掉那行即红（变异验证） |
| 12 | 生产 spawn 全覆盖（`spawn/output/status/wait_with_output`） | 源码文本闸门 | ✅ 已扩面（原先漏后两个） |
