...
| M12 | **跨层契约的判据必须落在「发射序 × 消费表」的联合断言上**。**生产者的测试与消费者的测试各自通过，证明不了它们之间还连着**。凡跨进程 / 跨语言契约（IPC 事件、序列化形状），必须有一条测试**同时消费两侧真实产物**；序应单一来源（共享 fixture 优先），**不得**在测试内重实现逻辑、也**不得**用手抄副本充当"真实序"（副本必然过期，且过期后仍绿） | 判据 = 该断言是否**同时读到两侧产物**（Rust 发射序 ↔ 前端消费表 / 两语言共读同一 fixture），且任一侧单方面改动即红；**本轮实证**：D6 修完后**换形式复发**——两侧单测全绿期间 seam 已断（破损态丢 6 条、末态卡 `checking`；修复态零丢、进度 `[10,50,90]`），详见 `docs/known-issues/v120-测试问题定位.md` §6.1；同源第二例 = D5b（`updates.rs` 改路径 + 前端 `setVersions` 覆盖）；闸门已由 `task-55` 落地（`updater.rs::frontend_transitions_accept_every_emitted_pair_per_branch`） |

> **编号说明**：本条记为 **M12**，编号**刻意不回填**。`M10` / `M11` 是**上一轮讨论过、
> 但从未落盘的候选编号**（全仓无任何落盘引用），而 `M12` 已被**三处交叉引用**
> （清册 H5 · `docs/known-issues/v120-测试问题定位.md` §6.1 · 本轮广播）。
> **放弃已存在的引用去迁就一个空编号，是把标识符稳定性换成编号美观**——不划算。
> 故 §8 现存条目为 **M1–M9 + M12**，跳号是预期结果，非缺漏。
> （lead 已裁定，2026-09-11。）
