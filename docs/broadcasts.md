...
- **结论**：维护者手动启动构建后实测 —— 顶栏**双击可最大化窗口**（2026-09-23）。这补齐了
  `180b52e`（补 `core:window:allow-toggle-maximize`）留下的最后一个待验项。
- **记录口径（诚实边界）**：落点未逐像素区分是否落在系统原生标题栏条带内（那条带双击本身
  也走系统缩放），故按「**用户可感行为已确认** + 本机制路径与拖动同源（同一 `dragDecisionFor`
  判定 + 已授权的 `toggle_maximize`）」记录，不写成「本机制路径已逐像素验证」。
- **发版时序口径（重要，留给后人的判例）**：v1.3.3 的 tag 已于同日切出并推送、CI 正在构建，
  **本确认不追改已切 tag 的发行正文** —— `docs/RELEASE_NOTES.md` 里 v1.3.3 那条
  「双击最大化本轮尚未确认」保持原样：它是 **tag 内文件**，Release 正文由 CI 从该修订提取，
  事后改 master 上同一段字只会让文件与已发布正文不一致（且该表述是**保守**的，不构成失实）。
  该口径随下一个版本的发版说明覆盖；若维护者希望在 GitHub Release 页面上即时反映，
  可在发布后手动编辑该 Release 的描述（一行，不影响 tag）。
- **附：dev 构建与正式包不能同时跑（本次踩到的坑）**：`tauri.conf.json` 的 identifier
  只有一个（`io.github.realguan.dsh-dock`），`resolve.rs` 里的 `.dev` 后缀只管**数据目录**
  ⇒ 正式包在跑时启动 `target/debug/dsh-dock` 会被 single-instance 插件接管并**静默退出**
  （无输出、无日志）。要跑 dev 构建先退出 `/Applications/DSH Dock.app`；或等 CI 出包后直接
  用 v1.3.3 装机替换（当前 /Applications 里仍是 **v1.3.2 缺陷版**）。
- **凭据**：ADR-0029 §7.4、ADR-0030 §5 已回写；仅文档改动，无代码变更；tag `v1.3.3`
  未被移动（`git ls-remote` 复核一致）。
