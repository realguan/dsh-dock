...
> **基准日期**：2026-09-15
> **2026-09-17 更新（本计划 R4/I4 已撤回）**：**SSH 远程工作区（I4 / §3.4 / §7 R4–R4d）**
> 的实现已由维护者裁定**整体移除**（代码、IPC、向导、登记册条目、AGENTS §6 写入例外全退），
> **未随任何版本发布**；ADR-0023 状态 = 已撤回。下方 §3.4 与 §7 的 R4 各行保留**当时的实施记录**
> 作为决策史，不再代表现行仓库状态；本计划其余部分（I1/I2/I3/R1–R3/R5）不受影响。
>
...
| **R4** ❌ 已撤回 | **I4 SSH 远程工作区向导**（父项；2026-09-15 拆为下面四刀并全部收口，**2026-09-17 整体移除、未发布**） | I1 的 `PatchFile` 内核（已就绪） | 是（原估 2 条；**实为 3 条**） | 🟠 中高 | 见 R4a–R4d。原判据中的「roadmap §4.8 的 capability 收敛」**未落地且已显式留白**（ADR-0023 §5 明列不在范围），已在 roadmap §4.8 落地记录与下方状态段写明 |
...
> **执行状态（2026-09-17 复核）**：**R4/R4a–R4d 已于本日整体移除**（未发布）——下表 R4 各行的
> 「已完成」是 2026-09-15 的实施记录，**不代表现行状态**；R1a/R1b/R2/R3/R5 仍然有效。
>
