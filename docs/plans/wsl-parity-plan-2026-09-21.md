...
| **P0-c** | ✅ **已完成**（`c849e2e` → `e972c21`）：① 客体备份可寻址（回传相对路径）② 记账世界身份 `wsl:<distro>`（不改 schema）③ state/dismiss/enter 三个客体孪生 + `guest::dsh_home_abs`（客体 home 供 `split_by_layer` 比对层序）+ `commands/boot.rs` 三处接线；旧拒绝助手与 A9 期读侧拒绝测试按语义演进替换 |
