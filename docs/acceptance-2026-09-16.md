...
| 编号 | 要你做什么 | 形式 |
|:--|:--|:--|
| **A1** | 四张能力卡完整过一眼（你只发了其中两张）——入口已迁至 Profile 详情 →「实验能力」 | 看一眼 |
...
| **S4** | 实验能力页改版（**清单/详情分栏**）：左栏四项一屏看全、右栏常驻详情、点行只换右栏（入口见 §0.2，2026-09-20 迁入 Profile 详情页） | 看一眼 |
| **S5** | 「多智能体协同」已由 dsh 官方插件页托管：**能力面板不再展示它**；在 dsh 插件页给本档启用后，插件列表应出现它的两层（「内置」+「层 N」+版本随 dsh、无控制面），与模板层同列；其余三类一处：层带「层 N」序标、用户装的层带「第三方」、面板装的实验层带「实验性 · <能力>」（一个包一行，不重复） | 看一眼 |
...
| **插件中心** | 内部两个子 Tab：`插件市场` / `已安装` |
| **Profile 详情** | 选中一个 profile 后的 Tab：**`插件列表`**（三类合一：内置 / 第三方 / 实验性，各带行内标记） / **`实验能力`**（A 组） / `MCP 扩展` / `Patch YAML` |
| **会话维护** | 会话列表；`已归档` 档位里有 `取消归档`（B 组） |
| **系统控制台** | 内部子 Tab `偏好与守护` → Preferences 的 **`插件安装源`** 区块（E 组） |

> **入口变更（2026-09-20，ADR-0028）**：A 组「实验能力」已自 `插件中心 → 实验能力`
> 子页**迁入 Profile 详情页**，与「插件列表」并列（档位 = 你选中的那个 Profile）；插件中心的
> 该子页**已下线**，其 Tab 只剩市场 / 已安装；原「外挂插件」更名「插件列表」。
> 下文 S4 / S5 / A 组各条的**验收步骤按新入口执行**，界面内容本身未变。

> **版面变更（2026-09-20，ADR-0028 第二批）**：Profile 详情的「底座组合」tab **已退役**——
> dsh 内置层并入「插件列表」（行内「层 N」序标承载组合序），与第三方插件、实验能力装的包
> 三类同列、各带行内标记（「内置」/「第三方」/「实验性 · <能力>」）。
> 实验性行的开关/卸载**只在其归属的能力面板**（行内「去开关」跳过去）；原 S5 的「底座组合」
> 一眼改在「插件列表」看（层描述那条：base/web-app 具体说，其余层中性说明）。
>
> **第三批裁定（2026-09-20）**：dsh 随安装自带的 optional bundle（Agent Teams 两层）
> 已由官方插件页托管——**「实验能力」面板不再展示它们**；在本档启用后，插件列表里与
> 模板层同列展示（「内置」+「层 N」+ 版本随 dsh，无控制面）。
...
| 0‴ | **S5 看一眼（2026-09-17 新增；2026-09-20 多轮改）**：① Profile 详情 →「实验能力」：**不应再看到「多智能体协同」**——dsh 已把它收为 optional bundle，开关只在 dsh 自己的插件页。② 到 **dsh 自己的插件页**给这个 profile 打开 Agent Teams 两层 → 回 Profile 详情 →「插件列表」（「底座组合」tab 已退役）：应多出两行 **「内置」**（无开关/卸载），版本列是「版本随 dsh」；描述不再张冠李戴。③ 同页核对其余三类：dsh-base / dsh-web-app 也是「内置」；你经 npm 装进 profile 的层（如 @openviking/dsh-memory-plugin）应标「第三方」、带完整开关/更新/卸载，**一个包只出现一行**（栈内重复折叠）；auto-review 这类面板装的实验层是「实验性 · 自动安全审查」、**不带内置**、行内有「去开关」。④ **能力的基座与后端是「一组」不是两行并列**：`@deepseek-ai/dsh-browser-use`（基座）在上、挂完整「实验性 · 浏览器操作」+「基座」标 +「去开关」；`…-playwright-mcp`（正在用的后端）**缩进挂在它下面**、只挂「后端」标；鼠标悬停任一行**整组一起亮**。⑤ **行首不再有「层 3 / 层 4」这类号标**（组合序保留在排列次序里：内置层在前） | 这是你几条裁定的落地结果；**dev 档引擎已升到 0.1.6-alpha.2、随包自带**，真机看这条即可 |
| 0″ | **S4 看一眼（2026-09-20 第五次改版：控制面/后果面拆分）**：Profile 详情 →「实验能力」。**一屏应看到全部能力**（左栏控制面：图标 · 名称 · 状态 · **生效后端的包名** · 开关；多变体的行尾有「切换后端」菜单按钮），右栏是选中那项的**后果面**——身份 → 进度/失败 → 后端对照（只读）→ 会发生什么 → 排障细节（默认折叠）→ 移除（沉底）；**点左栏的行只换右栏内容、左栏纹丝不动**；换后端 = 点行尾菜单选一个（要拆旧档会先确认）；把窗口拖窄（<1024）应变成"点进详情 + 返回清单" | 你打回的正是"一屏只能看到一个工具、点详情还加长卡片"，这次得你的眼睛确认 |
...
> 入口：**控制中心 → Profile 列表 → 选中一个 profile → Tab「实验能力」**
> （2026-09-20 起；原「插件中心 → 实验能力」子页已下线，见 §0.2）。四条能力：多智能体协同 / 浏览器操作 /
> 桌面控制 / 自动安全审查。改配置后需**重启该 Profile** 才生效（面板上有重启提示条）。

### A1 🖐 面板形态与状态真实

> **状态**：🖐 **待你验收**（证据与复跑命令见 §1.5）
- **步骤**：打开上述入口（2026-09-20 起：Profile 列表 → 选中 profile →「实验能力」）。
- **期望**（v4 控制面/后果面）：① 左栏一能力一行，各带状态徽标（未启用 / 已启用 / 已停用 /
  需要修复 / 后端冲突），第二行是**生效后端的官方包名**（完整折行不截断），行尾开关；
  ② 多变体的能力（浏览器操作 3 个后端、桌面控制 2 个）行尾有「切换后端」菜单——点开是
  各后端的完整包名 + 一句话说明，**当前生效的那个带「当前生效」徽标且不可点**，点别的
  （要拆旧档时）先弹确认框；③ 右栏只读：后端对照逐行列出（哪档在跑、谁被前置门挡）、
  「启用后会发生什么」、共同前置；④ 排障细节在右栏「实现细节（排障用）」里默认收起，
  展开能看到每步的包名、钉住的版本（`pkg@0.1.6-alpha.1`）、激活方式；
  ⑤ **不出现「多智能体协同」**（dsh 自带，已迁去官方插件页托管）；④ 里没有第二套开关。
- **判定**：☐ 待你验收
...
- **入口**：控制中心 → Profile 列表 → 选中一个 profile → Tab **实验能力**（2026-09-20 起；
  原「插件中心 → 实验能力」子页已下线）。
