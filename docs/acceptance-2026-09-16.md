...
| **S4** | 实验能力页改版：一档一行写**插件名**、共用基座与共同前置的呈现、行内状态用词、详情展开 | 看一眼 |
...
| 0″ | **S4 看一眼（2026-09-17 新增）**：插件中心 →「实验能力」。每档一行、行首是**插件名**；多档共用的基座包标「另装共用包」；各档共同前置只写一次；点「详情」展开实现细节与各包自带简介 | 你上一条裁定（名字用插件名 + 版面规范）的落地结果，需要你的眼睛 |
...
### S4 🖐 待你验收 实验能力页按「插件名 + 版面规范」重做（2026-09-17）

> **状态**：🖐 **待你验收**（你裁定「实际上就是插件，名字使用插件名就行了」+「这一页的 UI/UX 需要优化规范一下」）

- **入口**：控制中心 → 插件中心 → **实验能力**。
- **期望**：
  - **一档一行、行首即插件名**（如 `@deepseek-ai/dsh-experimental-browser-use-playwright-mcp`），
    不再出现「Web 档 / 复用已装的 cua-driver」这类我们发明的名字；
  - 多档共用基座包另起一行标「另装共用包 …」；各档**共同**的前置只在卡片上写一次（「各档共同前置」），
    行内只留该档**额外**的前置（如 Chrome DevTools 的「本机需安装 Chrome」）；
  - **行内状态徽标按该档自己的状态取词**：卡片说「已停用」时行内也是「已停用」（旧版会自相矛盾地写「已启用」）；
  - 「已包含」的档（Agent Teams 的自建档）不可点、不参与切换；"正在生效"的那一档有多档时才标；
  - 「详情」在「插件」节标右侧，展开后是排障细节（钉版本 / 激活方式 / 行 id）+ 各包**自带的官方简介**
    （未装时如实说「装好这条后会显示它自带的官方简介」）；
