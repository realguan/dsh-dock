...
| **A6** | 点一次「移除该行并重启」——你的 dev profile 现在**还是坏的** | 点一下 |
...
| **S1** | 安全模式（ADR-0025）：动作集/默认收起/影响文案有单测与门禁；真机 A/B——坏 profile + 只停用户层行的 overlay → **就绪**，原样启动 → exit 1；帮助参数顺序契约（`--patch` 必须在 app 参数前）单测 | ✅ 我已验收 |
| **S2** | 补完可见性：控制中心横幅 + 「退出安全模式并重启」+ 实验能力面板两源说明 + 只读 IPC `get_safe_mode_state`（四处同步/登记册 64）+ 门禁 `safeModeBanner.test.ts` | 🖐 待你验收（点两下） |
| **G1–G5** | ADR 0020–0024 全在且 `docs/adr/README.md` 索引 24 行含一行结论；登记册 **63** 条；复现点 20/21；今日广播 4 条；`AGENTS.md` **219** 行（≤250） | ✅ 我已验收（抽查项） |
...
| # | 项 | 为什么只能你来 |
|:--|:--|:--|
| 0 | **S2 点两下**：失败卡「安全模式启动」→ 控制中心横幅「退出安全模式并重启」 | 安全模式的完整回路只能手点 |
| 1 | **A6 点一次**「移除该行并重启」 | 你的 dev profile 现在仍是坏的（那两行还在）；按钮链路我已验证，等你点 |
...
### S1 ✅ 我已验收 启动失败时给两条真出路（不再只有必然失败的"重试"）
> **状态**：✅ **我已验收**（证据与复跑命令见 §1.5）

- **步骤**：让启动失败（例如手工写一条缺 config / 包已卸载的挂载行后重启）。
- **期望**：错误卡**默认收起**，首屏给：标题 + 一行摘要 + **动作**（每个动作旁一句"它会造成什么"）：
  `移除该行并重启` / `安全模式启动` / `安全模式（备份并放空 patch）`；**没有** `重试`。
- **已验**：分类（`apply`/`import` 两种措辞）＋ 动作集 ＋ 默认收起 ＋ 影响文案齐备，均有单测/门禁；
...
- **步骤**：① 在失败卡点「安全模式启动」→ 应真的重启并在启动时间线出现
