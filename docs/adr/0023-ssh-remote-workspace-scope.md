...
> **⚠ 状态（2026-09-17）：已撤回——功能整体移除，未随任何版本发布。**
> 维护者裁定「这个功能相关的代码逻辑都干掉」：`ssh_config.rs` / `ssh_profile.rs` /
> `ssh_remote.rs` / `commands/ssh.rs`、前端向导与三条 IPC 命令、网络面与读取域登记
> 于同日全部退役（登记册 §一/§二/§三 同步销记，AGENTS §6 的 app-bundle 写入例外一并回收）。
> 本 ADR 正文保留下方决策记录作为**决策史**（上游 SSH 家族的范围裁剪、五键契约、
> Web 视图非远端感知等结论仍然有效），但**不再作为现行实现口径**。

- **日期**：2026-09-15
- **状态**：**已接受**（2026-09-15 维护者裁定「评审通过」，含确认 §3 方案 C 的否决——
  即接受「Web 视图不随 provider 变远端感知」这一范围裁剪）——I4 增量据此解冻；
  **2026-09-17 撤回**（实现整体移除，见上）
- **提出人**：guan（AI 协作起草）
