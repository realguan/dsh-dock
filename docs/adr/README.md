...
| [0026](0026-safe-mode-via-config-disable.md) | 安全模式 = **在 profile 配置里把全部三方插件写成 `disabled: true`**（覆写前备份 + 一键用备份覆盖回去）；配置层即唯一真相源，`--patch` overlay 退役。**状态：已被 [0031](0031-remove-safe-mode-keep-patch-reset.md) 取代**（2026-09-24 维护者裁定；其「配置即状态」口径仍有效。原裁定：2026-09-16，同时推翻 ADR-0025 §4 的"三方 bundle 层停不掉"结论——真因是判据过宽） |
...
| [0028](0028-capability-panel-moves-into-profile.md) | 实验能力面板从插件中心迁入 Profile 详情页（与「插件列表」同页、受控档位、共享回读），插件中心 `official` 子页下线（单一入口）；「外挂插件」更名「插件列表」。**落地自查补记 §4.1**：受控化后换档入口搬出面板 ⇒ 原被 `Select disabled` 挡住的「在途换档」重新可达且旧令牌拦不住，补三道闸门（回读作废 / 落账复核 / 进度按发起档归属）。**第二批 §6/§6.1**：清单打标合并——「底座组合」tab 退役，内置 / 第三方 / 实验性三类一行（层序以「层 N」标保留且按包名折叠，实验性行只留「去开关」跳转＝单一入口）；「内置」判据锚定 dsh 源码（安装提供 = bundle∧非依赖，镜像 `removable` 规则；用户装的层是第三方），目录数据改父级受控（禁双源）；安装自带的 optional bundle（Agent Teams）不进场（官方插件页托管，启用后按内置层展示）；第五轮 §6.4：模块控制面/后果面拆分（行内即事实 + 切换后端溢出菜单，picked 中间态取消）；第六轮 §6.5：插件列表撤「层 N」号标 + 基座/provider 从属分组（anchor/缩进/hover 联动）。（2026-09-20 维护者裁定 + 真机截图回退后多轮修订） |
| [0029](0029-immersive-titlebar-marker-injection.md) | 沉浸式标题栏：官方客户端的桌面 CSS 真相源在 dsh 自己的 web 前端（`packages/client` 按 `html[data-platform='darwin']` 生效的一整批），壳里只因「标记没人打 + `-webkit-app-region` 是 Electron 行为」而休眠——壳新增注入脚本补打官方 preload 同款标记、按**计算样式**把 app-region 翻译成 `data-tauri-drag-region`（免疫 CSS Modules 哈希），主窗口 `#[cfg(target_os = "macos")]` 加 `TitleBarStyle::Overlay` + `Effect::Sidebar/Active`；不改 dsh 源码，Win/Linux 保持原生装饰，落空即静默降级。**Windows 一侧已由 [0030](0030-windows-keeps-native-decorations.md) 撤回** |
| [0030](0030-windows-keeps-native-decorations.md) | Windows **撤回**沉浸式标题栏、维持原生装饰（**2026-09-23 维护者裁定**，触发 = issue #16 真机事故「窗口没法移动 / 最大化 / 最小化 / 关闭」）：等价映射不成立（`decorations(false)` 在 tao 里摘掉 `WS_CAPTION \| WS_THICKFRAME` ⇒ 鼠标缩放与 Aero Snap 一并消失）、dsh 的 40px 拖拽带是伪元素 `::before` 挂不上 `data-tauri-drag-region`、自绘三控件既无 ACL 授权又被 `.catch` 吞错；注入脚本收窄回 macOS 单平台 + 新增「防复辟」闸门（负例实测）；ADR-0029 的 Windows 设想作废 |

| [0031](0031-remove-safe-mode-keep-patch-reset.md) | **删除安全模式**（一键停用 + 横幅记账 + `safe_mode.rs`），只保留「备份并放空 patch」兜底（2026-09-24 维护者裁定：上游 dsh 0.1.7-rc.1 实测插件不兼容/悬空行都不砖启动，ADR-0026 前提消失；仍砖的只有 YAML 语法坏与核心条目配置写坏两类用户层自写坏） |
## 维护约定
