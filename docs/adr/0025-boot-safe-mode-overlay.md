...
> **⚠️ 机制已退役（2026-09-16）：本 ADR 的 `--patch` overlay 方案被
> [ADR-0026](0026-safe-mode-via-config-disable.md) 取代**——安全模式改为"在 profile 配置里
> 停用全部三方插件 + 覆写前备份 + 一键用备份恢复"。退役原因：overlay 造成**两个真相源**
> （配置说启用 / 运行态说停用），维护者真机看到自相矛盾的界面，且明确裁定"安全模式就该是
> 配置里 disable"。**本文保留决策史**（调研、实测与工具链事实仍然有效），但：
> ① `--patch` 注入与 overlay 读写代码已删除；② 本文 §4 "三方 bundle 层行停不掉" 的结论
> **已被 ADR-0026 §2 实测推翻**（真因是判据把"被用户 patch 过的随包行"也算成了三方行）；
> ③ §7 的首屏单按钮裁定仍有效（ADR-0026 沿用：一个按钮 = 停用全部三方插件并启动）。
