...
function status(overrides: Partial<UpdateStatus> = {}): UpdateStatus {
  return {
    dsh: { current: "1.5.0", latest: null, newer: false, error: null, preview_latest: null },
    client: { current: "0.9.4", latest: null, newer: false, error: null, preview_latest: null },
    node: null,
    ...overrides,
  }
}
...
describe("lib/updateBanner.ts", () => {
  ...
  it("emits dsh and client banners in order when newer", () => {
    const s = status({
      dsh: { current: "1.5.0", latest: "1.6.0", newer: true, error: null, preview_latest: null },
      client: { current: "0.9.4", latest: "0.9.5", newer: true, error: null, preview_latest: null },
    })
  ...
  it("not-newer / missing-latest / error dimensions emit nothing", () => {
    const s = status({
      dsh: { current: "1.6.0", latest: "1.6.0", newer: false, error: null, preview_latest: null },
      client: { current: "0.9.4", latest: null, newer: false, error: "更新源不可达", preview_latest: null },
    })
