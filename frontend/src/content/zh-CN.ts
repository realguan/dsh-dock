...
export const t = {
  ...
  market: {
    ...
    // 官方实验室（2026-09-15，ADR-0020）：策展目录的呈现文案
    labTab: "官方实验室",
    labDesc: "dsh-dock 策展的官方实验性能力。安装顺序与「谁写配置行」由后端契约决定，此处如约呈现。",
    labPickProfile: "请选择目标 Profile",
    labLoadFailed: "读取官方目录失败",
    labInstallBtn: "安装",
    labInstalledBadge: "已装",
    labActivationAuto: "由 dsh 自动激活（不写配置行）",
    labActivationInsert: "需写入挂载行",
    labProgress: (done: number, total: number) => `进行中 ${done}/${total}`,
    labDone: "安装完成",
    labPartial: (done: number, total: number, err: string) => `已完成 ${done}/${total}，失败：${err}`,
    labResume: "继续剩余步骤",
    labReplaces: (pkg: string) => `将先移除同族 provider「${pkg}」（同族不能并列）`,
    labPinnedTo: (spec: string) => `钉版本：${spec}`,
