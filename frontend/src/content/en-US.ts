...
export const enUS: AppCopy = {
  ...
  market: {
    ...
    // Official Lab (2026-09-15, ADR-0020): copy for the curated catalog
    labTab: "Official Lab",
    labDesc: "Official experimental capabilities curated by dsh-dock. Install order and who writes the config row are decided by the backend contract and shown as-is.",
    labPickProfile: "Pick a target Profile",
    labLoadFailed: "Failed to load the official catalog",
    labInstallBtn: "Install",
    labInstalledBadge: "Installed",
    labActivationAuto: "Activated by dsh itself (no config row)",
    labActivationInsert: "Needs a mount row",
    labProgress: (done: number, total: number) => `Running ${done}/${total}`,
    labDone: "Install finished",
    labPartial: (done: number, total: number, err: string) => `Completed ${done}/${total}, failed: ${err}`,
    labResume: "Continue remaining steps",
    labReplaces: (pkg: string) => `Will first remove same-family provider "${pkg}" (they cannot coexist)`,
    labPinnedTo: (spec: string) => `Pinned: ${spec}`,
