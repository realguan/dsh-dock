...
export const enUS: AppCopy = {
  ...
  sessions: {
    ...
    unarchiveSuccess: "Session unarchived and moved back to the active list",
    // Progressive disclosure for long lists (2026-09-18 boundary fix)
    showMoreSessions: (n: number) => `Show older sessions (${n} more)`,
    // ==== 2026-09-18 Profiles UI convergence: migrated inline copy ====
    timeUnknown: "unknown",
    timeJustNow: "just now",
    timeMinutes: (n: number) => `${n} min ago`,
    timeHours: (n: number) => `${n} h ago`,
    timeDays: (n: number) => `${n} d ago`,
    repairFailed: "Repair failed",
    repairAllFailed: "Batch repair failed",
    openDirFailed: (reason: string) => `Failed to open directory: ${reason}`,
    loadingScanning: "Scanning DSH session records...",
    viewFlat: "Flat list",
    statProjects: "Workspace projects",
    statTotal: "Total sessions",
    statHealthy: "Healthy",
    statRunning: "Running",
    statNeedsRepair: "Need repair",
    copyIdTitle: "Copy session ID",
    groupMeta: (n: number, size: string) => `${n} sessions · ${size}`,
  },
  console: {
