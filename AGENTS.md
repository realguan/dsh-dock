...
2. **安装包不内置依赖**（Node / dsh）：首次使用联网由引擎自补齐；**唯一例外 = pnpm
   引导器随壳内置**（各平台一份；Windows 包**另带 linux-x64（glibc）份**供 WSL 客体
   ——原 **musl 静态份已于 2026-09-04 取消**，ADR-0010 §223；客体只支持 glibc 发行版）。
3. **平台覆盖不得擅自收窄**（2026-09-21 维护者裁定）：适配目标是 **macOS（arm64 /
   x86_64）· Windows（x64，含 WSL2 客体模式）· Linux（x64）**；**只要技术允许就必须
   都适配**——任何人不许自行判定「这个平台先不做 / 先不适配」。确有技术不可能 → 该面
   **显式报错并说明原因**（禁静默降级、禁假实现；先例 = WSL 客体档缺客体侧原语的命令，
   ADR-0016），而删 leg / 删上传 / 删发布下载属宪法级决策：**维护者裁定 + ADR**。
   机器闸门 `scripts/tests/test_release_platform_matrix.py` 守「矩阵 · 上传 · 下载」三处
   齐备（缺一即红）；用户可见矩阵以 README「下载与安装」表为唯一权威（禁双源）。
