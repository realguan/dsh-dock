//! safe_mode.rs —— 启动失败的**安全模式**（ADR-0025，2026-09-16 维护者裁定 A+ 口径）。
//!
//! ## 解决什么
//!
...
//!
//! ## 停用范围（A+：用户层行 + 第三方 bundle 层行）
//!
//! 保留随包发布的两层（[`KEEP_BUNDLES`]），其余**全部**停用：
//! 用户 patch 行（profile 层 / home 层，`contributed_by` 是文件路径）、第三方 bundle 层
//! 行（如 agent-team 的层）。官方桌面 app 的等价按钮只重置 `dsh.profile.bundles`
//! （`apps/desktop/src/project-manager.ts:335-344`）——**治不了 `cordis.patch.yml` 的
//! insert 行**，即本仓库这次踩的病，故不能照搬。
//!
//! ## 分层兜底（方案 B）
//!
